...
do
  ...
  vim.keymap.set('n', '<C-Up>', '5<C-w>+', { desc = 'Resize window up' })
  vim.keymap.set('n', '<C-Down>', '5<C-w>-', { desc = 'Resize window down' })
  vim.keymap.set('n', '<C-Left>', '5<C-w><', { desc = 'Resize window left' })
  vim.keymap.set('n', '<C-Right>', '5<C-w>>', { desc = 'Resize window right' })
...
do
  ...
  --
  --  Utility commands defined at the bottom of this section:
  --    :PackList              - list installed plugins
  --    :PackUpdate [name...]  - fetch updates and open the review buffer (all plugins if no names)
  --    :PackPreview [name...] - inspect plugin state and pending updates without fetching
  --    :PackReinstall {name}  - delete and re-clone a plugin (fixes broken installs)
  --    :PackNuke              - delete ALL plugins; restart nvim to reinstall everything
  --
  ...
  -- [[ vim.pack utility commands ]]
  -- Thin wrappers around the vim.pack API (see `:help vim.pack`).

  ---Complete installed plugin names for the commands below.
  ...
  -- List installed plugins (● = loaded in this session)
  vim.api.nvim_create_user_command('PackList', function()
    local plugins = vim.pack.get()
    table.sort(plugins, function(a, b) return a.spec.name < b.spec.name end)
    local lines = {}
    for _, p in ipairs(plugins) do
      table.insert(lines, ('%s %-35s %s'):format(p.active and '●' or '○', p.spec.name, p.spec.src or ''))
    end
    vim.notify(('%d plugins (● = active):\n%s'):format(#plugins, table.concat(lines, '\n')), vim.log.levels.INFO)
  end, { desc = 'List installed vim.pack plugins' })

  -- Update all plugins, or only the ones named as arguments.
  -- Opens a review buffer with the incoming changes: `:write` applies, closing aborts.
  vim.api.nvim_create_user_command(
    'PackUpdate',
    function(cmd) vim.pack.update(#cmd.fargs > 0 and cmd.fargs or nil) end,
    { nargs = '*', complete = complete_pack_names, desc = 'Fetch plugin updates and review (:write applies)' }
  )

  -- Same as PackUpdate but offline: shows current state and anything already
  -- fetched without touching the network.
  vim.api.nvim_create_user_command(
    'PackPreview',
    function(cmd) vim.pack.update(#cmd.fargs > 0 and cmd.fargs or nil, { offline = true }) end,
    { nargs = '*', complete = complete_pack_names, desc = 'Preview plugin state without fetching' }
  )

  -- Delete a plugin from disk and re-clone it with its original spec
  -- (version pin included). Build steps re-run via the PackChanged hook above.
  vim.api.nvim_create_user_command('PackReinstall', function(cmd)
    ...
    if not spec then
      vim.notify(('PackReinstall: no installed plugin named %q'):format(name), vim.log.levels.ERROR)
      return
    end
    vim.pack.del { name }
    vim.pack.add { spec }
    vim.notify(('Reinstalled %s. Restart nvim if it misbehaves.'):format(name), vim.log.levels.INFO)
  end, { nargs = 1, complete = complete_pack_names, desc = 'Delete and re-clone a plugin' })

  -- Delete ALL plugins from disk (asks for confirmation first). The vim.pack.add
  -- calls throughout the config re-clone everything on the next start, so expect
  -- that start to be slow, like the very first install.
  vim.api.nvim_create_user_command('PackNuke', function()
    local names = vim.tbl_map(function(p) return p.spec.name end, vim.pack.get())
    if #names == 0 then
      vim.notify('PackNuke: nothing installed', vim.log.levels.INFO)
      return
    end
    if vim.fn.confirm(('Delete all %d plugins from disk?'):format(#names), '&Yes\n&No', 2) ~= 1 then return end
    vim.pack.del(names)
    vim.notify(('Deleted %d plugins. Restart nvim to reinstall everything.'):format(#names), vim.log.levels.WARN)
  end, { desc = 'Delete all plugins from disk; restart nvim to reinstall' })
end
